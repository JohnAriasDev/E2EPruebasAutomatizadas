class PostSelector{
    ...
    getPageExcerptMessageError(){
        return cy.get('textarea[name="post-setting-custom-excerpt"]', { timeout: 20000 }).parent('.form-group').children('p.response');
    } 
    getPageMetadataTitleMessageError(){
        return cy.get('input[name="post-setting-meta-title"]', { timeout: 20000 }).parent('.form-group').children('p').children('span.word-count');
    }    
    getPageMetadataDescriptionMessageError(){
        return cy.get('textarea[name="post-setting-meta-description"]', { timeout: 20000 }).parent('.form-group').children('p').children('span.word-count');
    }    
