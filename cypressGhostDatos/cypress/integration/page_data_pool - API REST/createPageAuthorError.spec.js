...
describe("create a page with date error", () => {
  ...
  it("Create a page with author error", () => {   
    ...
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    postSelector.getTextAreaTitle().type(data_test.body[2].page_title);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    postSelector.getEditor().type(data_test.body[2].page_description);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    postSelector.getSettings().click({force: true});
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    //postSelector.getPageURL().clear({ force: true});
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    postSelector.getPageURL().clear().type(data_test.body[2].page_url);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    //postSelector.getPageDate().clear().type(data_test.body[2].page_date_pub);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    //postSelector.getPageTime().clear().type(data_test.body[2].page_time_pub);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
    //postSelector.getPageExcerpt().type(data_test.body[2].page_excerpt);
    //postSelector.getScreenShot(data.version4, data.scenarios.S9, index++);
