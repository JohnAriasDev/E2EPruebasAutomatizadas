...
describe("create member With note 500 character", () => {
    let data;
    const myOracleName = faker.name.firstName();
    const myOracle = faker.internet.email();
    const myOracleNote = (faker.lorem.paragraphs() + faker.lorem.paragraphs()) .replace(/[^A-Z0-9]/ig, "").substring(0,500);
  ...
  //@When
  it("Create member With note 500 character", () => {
